...
import torch
from model import MinkUNet, SPVCNN, MinkUNet_S, MinkUNet18, MinkUNet50, MinkUNet101
...
def make_model(config, load_path=None):
    ...
    if config['model_points'] == 'minkunet':
        ...
        model = MinkUNet_S(1, config["model_n_out"], config)
    elif config['model_points'] == 'minkunet18':
        model = MinkUNet18(1, config["model_n_out"], config)
    elif config['model_points'] == 'minkunet50':
        model = MinkUNet50(1, config["model_n_out"], config)
    elif config['model_points'] == 'minkunet101':
        model = MinkUNet101(1, config["model_n_out"], config)
    elif config['model_points'] == 'spvcnn':
        model = SPVCNN(4, config["model_n_out"], config)
