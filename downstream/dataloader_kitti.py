...
def custom_collate_fn_semkitti(list_data):
    """
...
class SemanticKITTIDataset(Dataset):
    ...
    def __init__(self, phase, config, transforms=None):
        ...
        if config['using_coord_shift']:
            self.coord_shift = np.array(config['semkitti_coord_shift'], dtype=np.float32).reshape(1, 3)
        else:
        ...
        self.scribbles = config['use_scribbles']
        print('Info: using scribbles: {}'.format(self.scribbles))
    ...
    def __getitem__(self, idx):
        ...
        pc = points[:, :3]
        if self.labels:
            if self.scribbles and self.phase == 'train':
