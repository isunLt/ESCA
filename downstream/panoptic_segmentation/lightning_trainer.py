...
class LightningDownstream(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self._config["loss"] == 'text_cosine':
            loss = self.text_cosine_loss(output_points, batch)
        else:
    ...
    def text_cosine_loss(self, outputs, batch):
        valid_voxel_mask, valid_voxel_text_sup = batch['valid_voxel_mask'], batch['valid_unique_text_sup']

        outputs = outputs[valid_voxel_mask]

        loss = (1 - torch.nn.CosineSimilarity()(outputs, valid_voxel_text_sup)).mean()
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.validation_step_outputs.append(c_matrix)
    ...
    def on_validation_epoch_end(self, ) -> None:
        c_matrix = sum(o for o in self.validation_step_outputs)
        # c_matrix = sum([o[1] for o in outputs])
