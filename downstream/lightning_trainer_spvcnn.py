...
class LightningDownstream(pl.LightningModule):
    ...
    def configure_optimizers(self):
        if self._config.get("lr_head", None) is not None:
            ...
            def is_final_head(key):
                return key.find('classifier_vox.') != -1
            param_group_head = [
    ...
    def training_step(self, batch, batch_idx):
        if self._config["freeze_layers"]:
            ...
            self.model.train()
        # cur = 0
        # for n in batch['len_batch']:
        #     visualize_utils.visualize_pcd(batch['Input_P'].F[cur:cur+n, :], target=batch["labels"].F.long()[cur:cur+n])
        #     cur = cur + n
        sparse_input = batch['Input_P'].cuda()
        target = batch["labels"].F.long().cuda(non_blocking=True)
        # sparse_input = SparseTensor(batch["sinput_F"], batch["sinput_C"])
        output_points = self(sparse_input)

        loss = self.criterion(output_points, target)
    ...
    def validation_epoch_end(self, outputs):
        ...
        self.log('best_IoU', self.best_mIoU, prog_bar=True, logger=True, sync_dist=False)
    ...
    @rank_zero_only
    def save_best(self):
        path = os.path.join(self.working_dir, "best_miou_model.pt")
        torch.save(
            {"model_points": self.model.state_dict(), "config": self._config}, path
        )