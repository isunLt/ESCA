...
def custom_collate_fn(list_data):
    ...
    if labelized:
        xyz, coords, feats, labels, evaluation_labels, inverse_indexes, lidar_token = input
    else:
        xyz, coords, feats, inverse_indexes, lidar_token = input
...
class NuScenesCDataset(Dataset):
    """
    Dataset returning a lidar scene and associated labels.
    """

    def __init__(self, phase, corrupt_type, level, config, transforms=None, cached_nuscenes=None):
        self.phase = phase
        self.labels = self.phase != "test"
        self.transforms = transforms
        self.voxel_size = config["voxel_size"]
        self.cylinder = config["cylindrical_coordinates"]
        self.corrupt_dataroot = os.path.join(str(config["corrupt_dataroot"]), corrupt_type, level)
        ...
        self.eval_labels = {
            ...
            23: 10, 24: 11, 25: 12, 26: 13, 27: 14, 28: 15, 29: 0, 30: 16, 31: 0, 41: 0, 42: 0, 43: 0, 44: 0
        }
    ...
    def __getitem__(self, idx):
        ...
        pcl_path = os.path.join(self.corrupt_dataroot, pointsensor["filename"])
        ...
        if self.labels:
            lidarseg_labels_filename = os.path.join(
                self.corrupt_dataroot, self.nusc.get("lidarseg", lidar_token)["filename"]
            )
            points_labels = np.fromfile(lidarseg_labels_filename, dtype=np.uint8)
            # visualize_utils.visualize_pcd(pc, target=points_labels, select_inds=(points_labels==41))
        ...
        if self.labels:
            ...
            return pc, discrete_coords, unique_feats, inverse_indexes, lidar_token


def make_data_loader(config, corrupt_type, level, phase, num_threads=0, cached_nuscenes=None):
    """
    ...
    dset = NuScenesCDataset(phase=phase, transforms=transforms, config=config, corrupt_type=corrupt_type, level=level, cached_nuscenes=cached_nuscenes)
