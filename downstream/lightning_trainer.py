...
class LightningDownstream(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        if self._config["freeze_layers"]:
            ...
            self.model.train()
        # for i in range(batch['sinput_C'].size(0)):
        #     mask = batch['sinput_C'][:, 0] == i
        #     visualize_utils.visualize_pcd(batch['sinput_C'][mask, 1:4], target=batch['labels'][mask])
