...
def spvcnn_collate_pair_fn(list_data):
    ...
    for batch_id in range(len(voxel)):
        ...
        batch_n_points.append(voxel[batch_id].C.shape[0])
        batch_n_pairings.append(pairing_points[batch_id].shape[0])
        offset += voxel[batch_id].C.shape[0]
    ...
    superpixels_batch = torch.tensor(np.concatenate(superpixels)) if superpixels is not None else None
    return {
...
class NuScenesMatchDatasetSPVCNN(Dataset):
    """
    ...
    def map_pointcloud_to_image(self, data, min_dist: float = 1.0):
        ...
        for i, camera_name in enumerate(camera_list):
            ...
            if self.superpixels_type == 'dinov2':
                ...
                sp = np.array(Image.open(os.path.join(self.superpixels_path, str(cam['token']) + '.png')))
            elif self.superpixels_type == 'slic':
                sp = np.array(Image.open(os.path.join(self.superpixels_path, str(cam['token']) + '.png')))
    ...
    def __getitem__(self, idx):
        ...
        # )
        _, indexes, inverse_indexes = sparse_quantize(coords_aug,
                                                      return_index=True,
                                                      return_inverse=True)
        # indexes here are the indexes of points kept after the voxelization
        pairing_points = inverse_indexes[pairing_points]

        unique_feats = np.concatenate([pc[indexes], intensity[indexes].reshape(-1, 1)], axis=-1)
