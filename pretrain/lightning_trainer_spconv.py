...
class LightningPretrainSpconv(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.decoupled_head:
            output_points, output_points_tmp = self.model_points(batch["voxels"], batch["coordinates"])
            output_points = interpolate_from_bev_features(batch["pc"], output_points, self.batch_size,
                                                          self.model_points.bev_stride)
            output_points_tmp = interpolate_from_bev_features(batch["pc"], output_points_tmp, self.batch_size,
                                                          self.model_points.bev_stride)
            ...
            sup_loss = self.loss_superpixels_average(batch, output_points, output_images)
