...
class NuScenesMatchDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for scene_idx in range(len(self.nusc.scene)):
            ...
            if scene["name"] in phase_scenes:
                ...
                if skip_counter % skip_ratio == 0:
                    ...
                    #     self.create_list_of_scans_from_pkl(scene)
                    # else:
                    #     self.create_list_of_scans(scene)
        # if phase in ['parametrizing', 'train']:
        #     self.list_keyframes.extend(self.list_keyframes)
        ...
        print('len(list_keyframes):', len(self.list_keyframes))
        ...
        for i, camera_name in enumerate(camera_list):
            ...
            if self.superpixels_type == 'dinov2':
                ...
                if os.path.exists(sp_path):
                    ...
                    if 'dino_mask' in sp_path and 'dino_mask_png' not in sp_path:
                        sp_path = sp_path.replace('dino_mask', 'dino_mask_png')
                    sp_path = sp_path.replace('_dino_mask.bin', '.png')
...
def make_data_loader(config, phase, num_threads=0):
    ...
    batch_size = config["batch_size"] // config["num_gpus"] // 2
