    ...
    def __init__(
        ...
        **kwargs,
        ...
        if phase in ("train", "val", "test"):
            ...
            # phase_scenes = list(
            #     set(create_splits_scenes()["train"])
            # )
        ...
        if phase == 'train':
            ...
            self.list_keyframes.extend(sweeps_pair_list['top_1'])
    ...
    def map_pointcloud_to_image(self, data, min_dist: float = 1.0):
        ...
        for i, camera_name in enumerate(camera_list):
            ...
            if self.superpixels_type == 'dinov2':
                sp = np.array(Image.open(os.path.join(self.superpixels_path, str(cam['token']) + '.png'))) - 1
