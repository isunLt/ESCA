...
class Res16UNetBase(ResNetBase):
    ...
    def __init__(self, in_channels, out_channels, config, D=3):
        ...
        self.decoupled_head = config.get('decoupled_head', False)
        # self.conditions = config.get('src_datasets', None)
        # if self.conditions is None:
        #     self.NORM_TYPE = NormType.PDBatchNorm
    ...
    def network_initialization(self, in_channels, out_channels, config, D):
        ...
        self.embed_dim = self.PLANES[7]

        self.final = conv(
            self.PLANES[7] * self.BLOCK.expansion, out_channels, kernel_size=1, stride=1, bias=True, D=D  # default bias=True
        )

        if self.decoupled_head:
            self.final_tmp = conv(
                self.PLANES[7] * self.BLOCK.expansion, out_channels, kernel_size=1, stride=1, bias=True, D=D
            )
        # self.final = conv(self.PLANES[7], 64, kernel_size=1, stride=1, bias=True, D=D)
        #
        # self.cls = torch.nn.Sequential(
        #     get_norm(self.NORM_TYPE, 64, D, bn_momentum),
        #     MinkowskiReLU(True),
        #     conv(
        #         64, 17, kernel_size=1, stride=1, bias=True, D=D
        #     )
        # )
    ...
    def decode_tmp(self, x):
        out = self.final_tmp(x)
        if self.normalize_feature:
            return SparseTensor(
                out.F / torch.norm(out.F, p=2, dim=1, keepdim=True),
                coordinate_map_key=out.coordinate_map_key,
    ...
    def forward(self, x):
        if self.decoupled_head:
            ...
            return self.decode(out).F, self.decode_tmp(out).F
        return self.decode(self.extract_feat(x))

    # def forward(self, x):
    #     out = self.calc_feat(x)
    #     out = self.final(out)
    #     if self.normalize_feature:
    #         return SparseTensor(
    #             out.F / torch.norm(out.F, p=2, dim=1, keepdim=True),
...
class Res16UNet34C_SMALL(Res16UNet34):
    PLANES = (32, 32, 64, 128, 128, 64, 48, 48)
...
class Res16UNet50C(Res16UNet50):
    PLANES = (32, 64, 128, 256, 256, 128, 96, 96)
...
class Res16UNet101C(Res16UNet101):
    PLANES = (32, 64, 128, 256, 256, 128, 96, 96)
