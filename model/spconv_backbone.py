...
class VoxelNet(VoxelBackBone8x):
    def __init__(self, in_channels, out_channels, config, D=3):
        ...
        if self.decoupled_head:
            self.final_tmp = spconv.SparseConv3d(
                128,
                out_channels // 1,
                1,
                stride=1,
    ...
    def forward(self, voxels, coordinates):
        ...
        sp_tensor_base = super(VoxelNet, self).forward(sp_tensor)
        sp_tensor = self.final(sp_tensor_base)
        ...
        if self.decoupled_head:
            sp_tensor_tmp = self.final_tmp(sp_tensor_base)
            sp_tensor_tmp = sp_tensor_tmp.replace_feature(nn.functional.normalize(sp_tensor_tmp.features, dim=1))
            sp_tensor_tmp = self.height_compression(sp_tensor_tmp)
            return sp_tensor, sp_tensor_tmp
        return sp_tensor
