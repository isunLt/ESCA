...
from model.dinov2.layers import (
    Mlp,
    PatchEmbed,
    SwiGLUFFNFused,
    MemEffAttention,
...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_get_last_n(self, x, n=1):
        ...
        for i, blk in enumerate(self.blocks):
            x = blk(x)
            # print(f"i={i} / {len(self.blocks)} - {len(self.blocks) - i} output {i in blocks_to_take}")
            if i in blocks_to_take:
                output["x"].append(self.norm(x))
                output["x_pre_norm"].append(x)
...
def _make_dinov2_model(
    ...
    **kwargs,
    ...
    model = vits__dict__[arch_name](**vit_kwargs)

    if pretrained:
        url = _DINOV2_BASE_URL + f"/{model_name}/{model_name}_pretrain.pth"
        state_dict = torch.hub.load_state_dict_from_url(
            url, model_dir="./dinov2_weights/", map_location="cpu"
        )
