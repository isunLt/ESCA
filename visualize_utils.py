...
def visualize_pcd(xyz, **kwargs):
    ...
    for k, v in kwargs.items():
        ...
        if k == "predict":
            ...
            heatmap = get_cmap('coolwarm')(v)[:, :3]
            heatmap = o3d.utility.Vector3dVector(heatmap)
    ...
    def render_heatmap_callback(viewer):
        if 'heatmap' in kwargs.keys():
            pcd.colors = heatmap
            viewer.update_geometry(pcd)
        else:
            print("No heatmap provided")
    ...
    viewer.register_key_callback(ord("H"), render_heatmap_callback)
    ...
    if kwargs.get('viewpoint', None) is not None:
        ...
        if os.path.exists(path):
            ...
            print('load view point file', path)
        else:
