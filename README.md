...
### Optional
1. To support SemanticKITTI, download it from the official [link](https://semantic-kitti.org/) and put it in `{project_root}/datasets/semantickitti`.
...
```bash
python pretrain.py --cfg_file config/slidr_cylinder3d.yaml --run_dir dinov2_supcon_supconintra_vse
```
2. Fine-tune the 3D backbone using the provided script:
```bash
